...
[6.6.2 Orders and task updates](#Oaou)<br>
...
[6.6.4.2 Vehicle receives an task with actions it cannot perform (e.g. lifting height higher than maximum lifting height, or lifting actions although no stroke is installed), or with fields that it cannot use (e.g. Trajectory)](#Vraowaicpeglhhtmlholaansii)<br>
...
Other interfaces required for operation between AGV and RCS (e.g., for exchanging map information, taking special skills freely into account with regard to path planning, etc.) or for communicating with other system components (e.g., external peripherals, fire protection gates, etc.) are not initially included in this document. 
...
The goal is to allow AGV with different characteristics (e.g., underrun tractor or fork lift AGV) to communicate with RCS in uniform language. 
This creates the basis for operating any combination of AGV in a RCS.
RCS provides orders and coordinates the AGV traffic.
...
The different messages are presented in tables describing the contents of the fields of the JSON that is sent as an task, state, etc.
...
If a variable is marked as optional, it means that it is optional for the sender because the variable might not be applicable in certain cases (e.g., when RCS sends an task to an AGV, some AGV plan their trajectory themselves and the field trajectory within the segment object of the task can be omitted). 
...
RCS shall only send optional information that the AGV supports.
...
topic | string | Topic (e.g. task or System State) see Cap. 6.5
...
---|---|---|---|---|---
task | RCS | AGV | Communication of driving orders from RCS to the AGV | mandatory | task.schema 
...
The graph representation in RCS contains restrictions, e.g., which AGV is allowed to traverse which segment.
...
Whenever possible, one location, e.g., a waiting position in front of fire door, should only have one point for all types of AGV.
...
![Figure 3 Graph representation in RCS and graph transmitted in orders](./assets/Figure3.png) 
>Figure 3 Graph representation in RCS and graph transmitted in orders
...
For traffic control and to accommodate resource constrained vehicles, the full transport task (which might consist of many points and segments) can be split up into many sub-orders, which are connected via their orderId and orderUpdateId. 
The process of updating an task is described in the next section.



### <a name="Oaou"></a> 6.6.2 Orders and task update 
...
For actions, deviation range, etc. the AGV must use the instructions provided in the first task (Figure 5, orderUpdateId 0).
...
To allow loops in orders (like going from point 1 to 2 and then back to 1) a sequenceId is assigned to the point and segment objects. 
...
After receiving the instantAction cancelOrder, the vehicle stops (based on its capabilities, e.g., right where it is or on the next point).
...
#### <a name="Vraowaicpeglhhtmlholaansii"></a> 6.6.4.2 Vehicle receives an task with actions it cannot perform (e.g. lifting height higher than maximum lifting height, or lifting actions although no stroke is installed), or with fields that it cannot use (e.g. Trajectory)
...
zoneSetId |  | string | Unique identifier of the zone set, that the AGV has to use for navigation or that was used by RCS for planning. <br> <br> Optional: Some RCS systems do not use zones.<br> Some AGV do not understand zones.<br> Do not add to message, if no zones are used. 
...
***nodePosition*** |  | JSON-object | point position. <br>Optional for vehicle-types that do not require the point position (e.g., line-guided vehicles).
...
*rotationAllowed* |  | boolean | “true”: rotation is allowed on the segment.<br>“false”: rotation is not allowed on the segment.<br><br>Optional:<br>No limit, if not set.
*maxRotationSpeed* | rad/s | float64| Maximum rotation speed<br><br>Optional:<br>No limit, if not set.
***trajectory*** |  | JSON-object | Trajectory JSON-object for this segment as a NURBS. <br>Defines the curve, on which the AGV should move between startNode and endNode.<br><br>Optional:<br>Can be omitted, if AGV cannot process trajectories or if AGV plans its own trajectory.
...
If the AGV supports actions other than driving, these actions are executed via the action field that is attached to either a point or an segment, or sent via the separate topic instantActions (see 6.9).

Actions that are to be executed on an segment must only run while the AGV is on the segment (see 6.10.2).

Actions that are triggered on points can run as long as they need to run. 
Actions on points should be self-terminating (e.g., an audio signal that lasts for five seconds or a pick action, that is finished after picking up a load) or should be formulated pairwise (e.g., activateWarningLights and deactivateWarningLights), although there may be exceptions. 
...
If there is no way to map some action to one of the actions of the following section, the AGV manufacturer can define additional actions that must be used by RCS.
...
action | counter action | Description | idempotent | Parameter | linked state | instant | point | segment
---|---|---|---|---|---|---|---|---
startPause | stopPause | Activates the pause mode. <br>A linked state is required, because many AGVs can be paused by using a hardware switch. <br>No more AGV driving movements - reaching next point is not necessary.<br>Actions can continue. <br>task is resumable. | yes | - | paused | yes | no | no 
...
stopCharging | startCharging | Deactivates the charging process to send a new task. <br>The charging process can also be interrupted by the vehicle / charging station, e.g., if the battery is full. <br>Battery state is only allowed to be “false”, when AGV is ready to receive orders. | yes | - |.batteryState.charging | yes | yes | no
...
pick | drop<br><br>(if automated) | Request the AGV to pick a load. <br>AGVs with multiple load handling devices can process multiple pick operations in parallel. <br>In this case, the paramater lhd needs to be present (e.g. LHD1). <br>The paramater stationType informs how the pick operation is handled in detail (e.g., floor location, rack location, passive conveyor, active conveyor, etc.). <br>The load type informs about the load unit and can be used to switch field for example (e.g., EPAL, INDU, etc). <br>For preparing the load handling device (e.g., pre-lift operations based on the height parameter), the action could be announced in the horizon in advance. <br>But, pre-Lift operations, etc., are not reported as running in the AGV state, because the associated point is not released yet.<br>If on an segment, the vehicle can use its sensing device to detect the position for picking the point. | no |lhd (string, optional)<br>stationType (string)<br>stationName(string, optional)<br>loadType (string) <br>loadId(string, optional)<br>height (float64) (optional)<br>defines bottom of the load related to the floor<br>depth (float64) (optional) for forklifts<br>side(string) (optional) e.g. conveyor | .load | no | yes | yes 
...
finePositioning | - | On a point, AGV will position exactly on a target.<br>The AGV is allowed to deviate from its point position.<br>On an segment, AGV will e.g. align on stationary equipment while traversing an segment.<br>InstantAction: AGV starts positioning exactly on a target. | yes | stationType(string, optional)<br>stationName(string, optional) | - | no | yes | yes
waitForTrigger | - | AGV has to wait for a trigger on the AGV (e.g. button press, manual loading). <br>RCS is responsible to handle the timeout and has to cancel the task if necessary. | yes | triggerType(string) | - | no | yes | no 
cancelOrder | - | AGV stops as soon as possible. <br>This could be immediately or on the next point. <br>Then the task is deleted. All actions are canceled. | yes | - | - | yes | no | no 
...
waitForTrigger | - | AGV is waiting for the Trigger | - | Trigger has been triggered. | waitForTrigger fails, if task has been canceled. 
cancelOrder | - | AGV is stopping or driving, until it reaches the next point. | - | AGV stands still and has canceled the task. | - 
...
instantActions must not conflict with the content of the AGV’s current task (e.g., instantAction to lower fork, while task says to raise fork).

Some examples for which instant actions could be relevant are: 
- pause the AGV without changing anything in the current task;
...
manufacturer | | string | Manufacturer of the AGV. 
serialNumber | | string | Serial number of the AGV.
actions [action] | | array | Array of actions that need to be performed immediately and are not part of the regular task. 
...
## <a name="TSfAtmc"></a> 6.10 Topic: "state" (from AGV to RCS)

The AGV-State will be transmitted on only one topic.
Compared to separate messages (e.g., for orders, battery-state and errors) using one topic will reduce the workload of the broker and RCS for handling messages, while also keeping the information about the AGV state synchronized.

AGV-State message will be published with occurrence of relevant events or at the latest every 30s via MQTT-broker to RCS. 

Events that trigger the transmission of the state message are:
- Receiving an task 
- Receiving an task update 
- Changes in the load status 
- Errors or warnings 
- Driving over a point 
...
If two events correlate with each other (e.g., the receiving of a new task usually forces an update of the point- and edgeStates; as does the driving over a point), it is sensible to trigger one state update instead of multiple.



### <a name="CaLe"></a> 6.10.1 Concept and Logic 

The task progress is tracked by the `nodeStates` and `edgeStates`. 
Additionally, if the AGV is able to derive its current position, it can publish its position via the “position” field.

If the AVG plans the path by itself, it must communicate its calculated trajectory (including base and horizon) in the form of a NURBS via the `trajectory` object in the state message, unless RCS cannot use this field and it was agreed during integration, that this field must not be sent.
After points are released by RCS, the AGV is not allowed to change its trajectory.

The `nodeStates` and `edgeStates` includes all points/segments, that the AGV still must traverse.

![Figure 12 task Information provided by the state topic. Only the ID of the last point and the remaining points and segments are transmitted](./assets/Figure12.png) 
>Figure 12 task Information provided by the state topic. Only the ID of the last point and the remaining points and segments are transmitted



### <a name="Tonaeletoa"></a> 6.10.2 Traversal of points and entering/leaving segments, triggering of actions 

The AGV decides on its own, when a point should count as traversed.
...
The AGV reports the traversal of a point by removing its `nodeState` from the `nodeStates` array and setting the `lastNodeId`, `lastNodeSequenceNumber` to the traversed node’s values.

As soon as the AGV reports the point as traversed, the AGV must trigger the actions associated with the point, if any.

The traversal of a point also marks the leaving of the segment leading up to the point. 
The segment must then be removed from the `edgeStates` and the actions that were active on the segment must be finished.

The traversal of the point also marks the moment, when the AGV enters the following segment, if there is one.
The segments actions must now be triggered. 
An exception to this rule is, if the AGV has to pause on the segment (because of a soft or hard blocking segment, or otherwise) – then the AGV enters the segment after it begins moving again.

![Figure 13 nodeStates, edgeStates, actionStates during task handling](./assets/Figure13.png)
>Figure 13 nodeStates, edgeStates, actionStates during task handling
...
The AGV can submit arbitrary additional information to RCS via the `information` array.
It is up to the AGV how long it reports information via an information message.

RCS must not use the info messages for logic, it must only be used for visualization and debugging purposes.
...
manufacturer | | string | Manufacturer of the AGV.
serialNumber | | string | Serial number of the AGV.
orderId|  | string | Unique task identification of the current task or the previous finished task. <br>The orderId is kept until a new task is received. <br>Empty string (""), if no previous orderId is available. 
orderUpdateId |  | uint32 | task Update Identification to identify, that an task update has been accepted by the AGV. <br>“0” if no previous orderUpdateId is available. 
*zoneSetId* |  |string | Unique ID of the zone set, that the AGV currently uses for path planning. <br>Must be the same as the one used in the task, otherwise the AGV has to reject the task.<br><br>Optional: If the AGV does not use zones, this field can be omitted.
lastNodeId |  | string | point ID of last reached point or, if AGV is currently on a point, current point (e.g., „node7”). Empty string (""), if no lastNodeId is available.
lastNodeSequenceId |  | uint32 | Sequence ID of the last reached point or, if AGV is currently on a point, Sequence ID of current point. <br>"o" if no lastNodeSequenced is available. 
**nodeStates [nodeState]** |  |array | Array of nodeState-Objects, that need to be traversed for fulfilling the task<br>(empty list if idle)
**edgeStates [edgeState]** |  |array | Array of edgeState-Objects, that need to be traversed for fulfilling the task<br>(empty list if idle)
...
*paused* |  | boolean | “true”: AGV is currently in a paused state, either because of the push of a physical button on the AGV or because of an instantAction. <br>The AGV can resume the task.<br><br>“false”: The AGV is currently not in a paused state.
*newBaseRequest* |  | boolean | “true”: AGV is almost at the end of the base and will reduce speed, if no new base is transmitted. <br>Trigger for RCS to send a new base.<br><br>“false”: no base update required.
...
**actionStates [actionState]** |  | array | Contains a list of the current actions and the actions, which are yet to be finished. <br>This may include actions from previous points, that are still in progress.<br><br>When an action is completed, an updated state message is published with actionStatus set to finished and if applicable with the corresponding resultDescription. <br><br>The action state is kept until a new task is received.
...
***information [info]*** |  | array | Array of info-objects. <br>An empty array indicates, that the AGV has no information. <br>This should only be used for visualization or debugging – it must not be used for logic in RCS.
...
nodeId |  | string | Unique point identification.
sequenceId |  | uint32 | sequenceId to discern multiple points with same nodeId.
*nodeDescription* |  | string | Additional information on the point.
***nodePosition*** |  | JSON-object | point position. <br>The object is defined in chapter 6.6 <br>Optional: <br>RCS has this information. <br>Can be sent additionally, e. g. for debugging purposes.
released<br><br>}|  | boolean | “true” indicates that the point is part of the base.<br>“false” indicates that the point is part of the horizon.

Object structure | Unit | Data type | Description 
---|---|---|---
**edgeState** { |  | JSON-object |  |
edgeId |  | string | Unique segment identification.
sequenceId |  | uint32 | sequenceId to differentiate between multiple segments with the same edgeId.
*edgeDescription* |  | string | Additional information on the segment.
released |  | boolean | “true” indicates that the segment is part of the base.<br>“false” indicates that the segment is part of the horizon.
***trajectory*** <br><br>} |  | JSON-object | The trajectory is to be communicated as a NURBS and is defined in chapter 6.4<br><br>Trajectory segments are from the point, where the AGV starts to enter the segment, until the point, where it reports, that the next point was traversed.
...
*actionType* |  | string | actionType of the action.<br><br>Optional: Only for informational or visualization purposes. task knows the type.
*actionDescription* |  | string | Additional information on the current action. 
actionStatus |  | string | Enum {WAITING; INITIALIZING; RUNNING; PAUSED; FINISHED; FAILED}<br><br>WAITING: waiting for the trigger<br>(passing the mode, entering the segment)<br> PAUSED: paused by instantAction or external trigger<br>FAILED: action could not be performed. 
...
---|---
AUTOMATIC | AGV is under full control of RCS. <br>AGV drives and executes actions based on orders from RCS.
SEMIAUTOMATIC | AGV is under control of RCS.<br> AGV drives and executes actions based on orders from RCS. <br>The driving speed is controlled by the HMI (speed can’t exceed the speed of automatic mode).<br>The steering is under automatic control (non-safe HMI possible).
MANUAL | RCS is not in control of the AGV. <br>Supervisor doesn’t send driving task or actions to the AGV. <br>HMI can be used to control the steering and velocity and handling device of the AGV. <br>Location of the AGV is send to RCS. <br>When AGV enters or leaves this mode, it immediately clears all the orders (safe HMI required).
SERVICE | RCS is not in control of the AGV. <br>RCS doesn’t send driving task or actions to the AGV. <br>Authorized personal can reconfigure the AGV. 
TEACHIN | RCS is not in control of the AGV. <br>Supervisor doesn’t send driving task or actions to the AGV. <br>The AGV is being taught, e.g., mapping is done by a RCS.



## <a name="actionStates"></a> 6.11 actionStates

When an AGV receives an `action` (either attached to a `point` or `segment` or via an `instantAction`), it must represent this `action` with an `actionState` in its `actionStates` array.
...
---|---
WAITING | Action was received by AGV but the point where it triggers was not yet reached or the segment where it is active was not yet entered.
...
The task of multiple actions in a list define the sequence, in which those actions are to be executed. 
...
If there are multiple actions on the same point with different blocking types, Figure 15 describes how the AGV should handle these actions.
...
Thus, RCS can detect a disconnection event by subscribing the connection topics of all AGV.
...
The factsheet also includes information about AGV communication interfaces which are required for the integration of an AGV type series into a VDA-5050-compliant RCS.
...
The factsheet is both intended as a human-readable document and for machine processing, e.g., an import by RCS application, and thus is specified as a JSON document.
...
| &emsp;*topicSerialLen*              | uint32        | Maximum length of serial-number part in MQTT-topics.<br/><br/>Affected parameters:<br/>task.serialNumber<br/>instantActions.serialNumber<br/>state.SerialNumber<br/>visualization.serialNumber<br/>connection.serialNumber   |
| &emsp;*topicElemLen*                | uint32        | Maximum length of all other parts in MQTT-topics.<br/><br/>Affected parameters:<br/>task.timestamp<br/>task.version<br/>task.manufacturer<br/>instantActions.timestamp<br/>instantActions.version<br/>instantActions.manufacturer<br/>state.timestamp<br/>state.version<br/>state.manufacturer<br/>visualization.timestamp<br/>visualization.version<br/>visualization.manufacturer<br/>connection.timestamp<br/>connection.version<br/>connection.manufacturer |
| &emsp;*idLen*                       | uint32        | Maximum length of ID-Strings.<br/><br/>Affected parameters:<br/>task.orderId<br/>task.zoneSetId<br/>point.nodeId<br/>nodePosition.mapId<br/>action.actionId<br/>segment.edgeId<br/>segment.startNodeId<br/>segment.endNodeId |
| &emsp;*idNumericalOnly*             | boolean          | If "true" ID-strings need to contain numerical values only. |
| &emsp;*enumLen*                     | uint32        | Maximum length of ENUM- and Key-Strings.<br/><br/>Affected parameters:<br/>action.actionType action.blockingType<br/>segment.direction<br/>actionParameter.key<br/>state.operatingMode<br/>load.loadPosition<br/>load.loadType<br/>actionState.actionStatus<br/>error.errorType<br/>error.errorLevel<br/>errorReference.referenceKey<br/>info.infoType<br/>info.infoLevel<br/>safetyState.eStop<br/>connection.connectionState                                               |
| &emsp;*loadIdLen*                   | uint32        | Maximum length of loadId Strings |
| }                             |               |                                  |
| **maxArrayLens** {            | JSON-object   | Maximum lengths of arrays.                                 |
| &emsp;*task.points*                 | uint32        | Maximum number of points per task processable by the AGV.  |
| &emsp;*task.segments*                 | uint32        | Maximum number of segments per task processable by the AGV.  |
| &emsp;*point.actions*                | uint32        | Maximum number of actions per point processable by the AGV. |
| &emsp;*segment.actions*                | uint32        | Maximum number of actions per segment processable by the AGV. |
...
| &emsp;*state.nodeStates*            | uint32        | Maximum number of nodeStates sent by the AGV, maximum number of points in base of AGV. |
| &emsp;*state.edgeStates*            | uint32        | Maximum number of edgeStates sent by the AGV, maximum number of segments in base of AGV. |
...
| &emsp;minOrderInterval              | float32       | [s], Minimum interval sending task messages to the AGV.        |
...
| {            |               |                  |
| &emsp;parameter    | string        | Full name of optional parameter, e.g. “*task.points.nodePosition.allowedDeviationTheta”*.|
...
| &emsp;actionScopes | array of enum  | List of allowed scopes for using this action-type.<br/><br/>INSTANT: usable as instantAction.<br/>point: usable on points.<br/>segment: usable on segments.<br/><br/>For example: ```[„INSTANT“, „NODE“]```|
...
If an error occurs due to an erroneous task, the AGV should return a meaningful error reference in the fields errorReference (see [6.10.6 Implementation](#errorReferenceImpl)).
This can include the following information:

- headerId
- Topic (task or instantAction)
- orderId and orderUpdateId if error was caused by an task update 
...
Concept | Description 
---|---
Free navigation AGV | Vehicles that use a map to plan their own path. <br>RCS sends only start and destination coordinates.<br>The vehicle sends its path to RCS.<br>When the communication to RCS is broken off, the vehicle is able to continue its journey.<br>Free-navigation vehicles may be allowed to bypass local obstacles.<br>It may also be possible that a fine adjustment of the receiving/dispensing position by the vehicle itself is carried out.
Guided vehicles (physical or virtual) | Vehicles that get their path sent by RCS. <br>The calculation of the path takes place in RCS.<br>When communication to RCS is broken off, the vehicle terminates its released points and segments (the "base") and then stops.<br>Guided vehicles may be allowed to bypass local obstacles.<br>It may also be possible that a fine adjustment of the receiving/dispensing position by the vehicle itself is carried out.
Central map | The maps that will be held centrally in RCS.<br> This is initially created and then used.<br> A future version of the interface will make it possible to transfer this map to the vehicles (e.g., for free navigation).
